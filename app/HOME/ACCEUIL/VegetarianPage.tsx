// app/HOME/RECIPES/VegetarianPage.tsx - Page dédiée aux recettes végétariennes
...
interface VegetarianPageProps {
  onKitchenPress?: () => void;
  onProfilePress?: () => void;
  onHomePress?: () => void;
}

export default function VegetarianPage({ onKitchenPress, onProfilePress, onHomePress }: VegetarianPageProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [favorites, setFavorites] = useState<number[]>([]);

  // Fonction pour récupérer les recettes végétariennes depuis TheMealDB
  const fetchVegetarianRecipes = async () => {
    setLoading(true);
    try {
      // Utilisons "Pasta" qui existe dans TheMealDB (souvent végétarien)
          ...
          data.meals.slice(0, 12).map(async (meal: any) => {
            try {
              ...
              return {
                ...
                readyInMinutes: Math.floor(Math.random() * 50) + 25, // Temps 25-75 min
      ...
      console.error('Erreur lors de la récupération des recettes végétariennes:', error);
      setRecipes([
        { id: 1, title: 'Risotto aux champignons', image: '', readyInMinutes: 45, servings: 3, cuisines: ['Italienne'], instructions: 'Faites revenir les champignons, ajoutez le riz et le bouillon progressivement.', ingredients: ['Riz Arborio', 'Champignons', 'Bouillon', 'Parmesan', 'Beurre'] },
        { id: 2, title: 'Curry de lentilles', image: '', readyInMinutes: 60, servings: 4, cuisines: ['Indienne'], instructions: 'Faites cuire les lentilles avec les épices et les légumes.', ingredients: ['Lentilles', 'Oignons', 'Tomates', 'Épices', 'Riz'] },
        { id: 3, title: 'Ratatouille provençale', image: '', readyInMinutes: 55, servings: 4, cuisines: ['Française'], instructions: 'Faites mijoter les légumes avec de l\'huile d\'olive et des herbes.', ingredients: ['Aubergines', 'Courgettes', 'Tomates', 'Poivrons', 'Herbes'] }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.contentContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>🌱 Végétarien</Text>
          <Text style={styles.subtitle}>Des recettes 100% végétales et délicieuses</Text>
        </View>
        ...
        <View style={styles.recipesSection}>
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Chargement des recettes végétariennes...</Text>
            </View>
          ) : (
