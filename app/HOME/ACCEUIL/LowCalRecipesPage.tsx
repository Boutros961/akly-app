// app/HOME/RECIPES/LowCalRecipesPage.tsx - Page dédiée aux recettes faibles en calories
...
interface LowCalRecipesPageProps {
  onKitchenPress?: () => void;
  onProfilePress?: () => void;
  onHomePress?: () => void;
}

export default function LowCalRecipesPage({ onKitchenPress, onProfilePress, onHomePress }: LowCalRecipesPageProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [favorites, setFavorites] = useState<number[]>([]);

  // Fonction pour récupérer les recettes faibles en calories depuis TheMealDB
  const fetchLowCalRecipes = async () => {
    setLoading(true);
    try {
      // Utilisons "Chicken" qui existe dans TheMealDB (plus léger que le bœuf)
          ...
          data.meals.slice(0, 12).map(async (meal: any) => {
            try {
              ...
              return {
                ...
                readyInMinutes: Math.floor(Math.random() * 40) + 20, // Temps 20-60 min
      ...
      console.error('Erreur lors de la récupération des recettes faibles en calories:', error);
      setRecipes([
        { id: 1, title: 'Salade composée légère', image: '', readyInMinutes: 25, servings: 2, cuisines: ['Française'], instructions: 'Lavez et coupez les légumes, ajoutez une vinaigrette légère.', ingredients: ['Laitue', 'Tomates', 'Concombres', 'Radis', 'Vinaigrette'] },
        { id: 2, title: 'Soupe de légumes', image: '', readyInMinutes: 45, servings: 4, cuisines: ['Française'], instructions: 'Faites cuire les légumes dans du bouillon, mixez et assaisonnez.', ingredients: ['Carottes', 'Poireaux', 'Pommes de terre', 'Bouillon', 'Herbes'] },
        { id: 3, title: 'Quinoa aux légumes', image: '', readyInMinutes: 35, servings: 3, cuisines: ['Internationale'], instructions: 'Faites cuire le quinoa, ajoutez les légumes sautés.', ingredients: ['Quinoa', 'Légumes', 'Huile d\'olive', 'Herbes', 'Citron'] }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.contentContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>🥗 Faible en Calories</Text>
          <Text style={styles.subtitle}>Des recettes légères et équilibrées</Text>
        </View>
        ...
        <View style={styles.recipesSection}>
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Chargement des recettes légères...</Text>
            </View>
          ) : (
