// app/HOME/RECIPES/AppetizersPage.tsx - Page dédiée aux apéritifs
...
interface AppetizersPageProps {
  onKitchenPress?: () => void;
  onProfilePress?: () => void;
  onHomePress?: () => void;
}

export default function AppetizersPage({ onKitchenPress, onProfilePress, onHomePress }: AppetizersPageProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [favorites, setFavorites] = useState<number[]>([]);

  // Fonction pour récupérer les apéritifs depuis TheMealDB
  const fetchAppetizers = async () => {
    ...
    try {
          ...
          data.meals.slice(0, 12).map(async (meal: any) => {
            try {
              ...
              return {
                ...
                readyInMinutes: Math.floor(Math.random() * 25) + 15, // Temps 15-40 min
      ...
      console.error('Erreur lors de la récupération des apéritifs:', error);
      setRecipes([
        { id: 1, title: 'Bruschetta à la tomate', image: '', readyInMinutes: 15, servings: 4, cuisines: ['Italienne'], instructions: 'Grillez le pain, ajoutez la tomate et le basilic.', ingredients: ['Pain de campagne', 'Tomates', 'Basilic', 'Huile d\'olive', 'Ail'] },
        { id: 2, title: 'Guacamole maison', image: '', readyInMinutes: 10, servings: 6, cuisines: ['Mexicaine'], instructions: 'Écrasez l\'avocat, ajoutez les autres ingrédients.', ingredients: ['Avocats', 'Oignon', 'Tomate', 'Citron vert', 'Coriandre'] },
        { id: 3, title: 'Tartinades variées', image: '', readyInMinutes: 20, servings: 8, cuisines: ['Française'], instructions: 'Préparez différentes tartinades avec des légumes.', ingredients: ['Fromage frais', 'Légumes', 'Herbes', 'Pain', 'Beurre'] }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.contentContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>🍽️ Apéritifs</Text>
          <Text style={styles.subtitle}>Des entrées délicieuses pour commencer le repas</Text>
        </View>
        ...
        <View style={styles.recipesSection}>
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Chargement des apéritifs...</Text>
            </View>
          ) : (
