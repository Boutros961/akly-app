// app/HOME/RECIPES/ProteinRecipesPage.tsx - Page dédiée aux recettes protéinées
...
interface ProteinRecipesPageProps {
  onKitchenPress?: () => void;
  onProfilePress?: () => void;
  onHomePress?: () => void;
}

export default function ProteinRecipesPage({ onKitchenPress, onProfilePress, onHomePress }: ProteinRecipesPageProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [favorites, setFavorites] = useState<number[]>([]);

  // Fonction pour récupérer les recettes protéinées depuis TheMealDB
  const fetchProteinRecipes = async () => {
    setLoading(true);
    try {
          ...
          data.meals.slice(0, 12).map(async (meal: any) => {
            try {
              ...
              return {
                ...
                readyInMinutes: Math.floor(Math.random() * 60) + 30, // Temps 30-90 min
      ...
      console.error('Erreur lors de la récupération des recettes protéinées:', error);
      setRecipes([
        { id: 1, title: 'Steak grillé à la française', image: '', readyInMinutes: 45, servings: 2, cuisines: ['Française'], instructions: 'Faites griller le steak à votre goût. Laissez reposer avant de servir.', ingredients: ['Steak de bœuf', 'Beurre', 'Ail', 'Thym', 'Sel et poivre'] },
        { id: 2, title: 'Poulet rôti aux herbes', image: '', readyInMinutes: 60, servings: 4, cuisines: ['Française'], instructions: 'Préchauffez le four à 180°C. Assaisonnez le poulet et faites-le rôtir.', ingredients: ['Poulet', 'Herbes de Provence', 'Ail', 'Beurre', 'Sel et poivre'] },
        { id: 3, title: 'Poisson en papillote', image: '', readyInMinutes: 35, servings: 2, cuisines: ['Française'], instructions: 'Enveloppez le poisson dans du papier aluminium avec des légumes.', ingredients: ['Filet de poisson', 'Légumes', 'Citron', 'Herbes', 'Huile d\'olive'] }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.contentContainer}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>🥩 Recettes Protéinées</Text>
          <Text style={styles.subtitle}>Des plats riches en protéines pour se muscler</Text>
        </View>
        ...
        <View style={styles.recipesSection}>
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Chargement des recettes protéinées...</Text>
            </View>
          ) : (
