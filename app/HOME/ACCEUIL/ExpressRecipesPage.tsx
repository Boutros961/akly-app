// app/HOME/RECIPES/ExpressRecipesPage.tsx - Page dédiée aux recettes express
...
interface ExpressRecipesPageProps {
  onKitchenPress?: () => void;
  onProfilePress?: () => void;
  onHomePress?: () => void;
}

export default function ExpressRecipesPage({ onKitchenPress, onProfilePress, onHomePress }: ExpressRecipesPageProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [favorites, setFavorites] = useState<number[]>([]);

  // Fonction pour récupérer les recettes express depuis TheMealDB
  const fetchExpressRecipes = async () => {
    setLoading(true);
    try {
      // Récupérer des recettes rapides (poulet et poisson)
      ...
      if (data.meals) {
          ...
          data.meals.slice(0, 12).map(async (meal: any) => {
            try {
              ...
              return {
                ...
                readyInMinutes: Math.floor(Math.random() * 30) + 10, // Temps rapide 10-40 min
        ...
        // Filtrer les recettes null et les ajouter
        const validRecipes = detailedRecipes.filter(recipe => recipe !== null);
        setRecipes(validRecipes);
      }
    } catch (error) {
      console.error('Erreur lors de la récupération des recettes express:', error);
      // Recettes par défaut si l'API ne fonctionne pas
      setRecipes([
        { id: 1, title: 'Salade fraîche du jardin', image: '', readyInMinutes: 10, servings: 2, cuisines: ['Française'], instructions: 'Lavez et coupez les légumes. Mélangez avec l\'huile d\'olive et le citron.', ingredients: ['Laitue', 'Tomates', 'Concombres', 'Huile d\'olive', 'Citron'] },
        { id: 2, title: 'Omelette aux champignons', image: '', readyInMinutes: 8, servings: 1, cuisines: ['Française'], instructions: 'Battez les œufs, ajoutez les champignons et faites cuire à feu doux.', ingredients: ['Œufs', 'Champignons', 'Beurre', 'Persil', 'Sel et poivre'] },
        { id: 3, title: 'Guacamole maison', image: '', readyInMinutes: 5, servings: 6, cuisines: ['Mexicaine'], instructions: 'Écrasez l\'avocat, ajoutez les autres ingrédients et mélangez.', ingredients: ['Avocats', 'Oignon', 'Tomate', 'Citron vert', 'Coriandre'] }
      ]);
    } finally {
      setLoading(false);
    }
    ...
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.contentContainer}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>⚡ Recettes Express</Text>
          <Text style={styles.subtitle}>Des recettes rapides et délicieuses</Text>
        </View>
        ...
        <View style={styles.recipesSection}>
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Chargement des recettes express...</Text>
            </View>
          ) : (
